...
- Brand typography uses licensed GT America faces per [Font system](./docs/FONTS.md): they are pinned by sha256 in `config/brand-fonts.json`, synced from private `putio-static` with `pnpm roku fonts-setup`, and **never committed** — `pnpm verify` fails if git ever tracks an `.otf`/`.ttf`/`.ttc`. A clone without them is fully working: packaging bundles `fonts/` only when present and compiles `buildConfigBrandFontsAvailable()` into `source/BuildConfig.brs` so the runtime falls back to `font:*SystemFont`. `pnpm roku fonts-check` is intentionally outside `pnpm verify`
